...
make_archive() {
	...
	if [ -n "${ROOT_NAME}" ]; then
		local ACTUAL_ROOT="${ROOT}/${ROOT_NAME}";
		mkdir -p "${ACTUAL_ROOT}";
		mv "${ROOT}"/* "${ACTUAL_ROOT}" >/dev/null 2>&1; # Would use -u, but -u lies and actually matches against equally old files as well (> != >=, GNU). Bummer.
	fi
